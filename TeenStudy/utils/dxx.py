...
async def hubei(user_id: int) -> dict:
    ...
    url += f"&username={name}"
    url += "&phone=" + "未知"
    url += f"&city={university}"
    url += f"&danwei2={organization}"
    url += f"&danwei={college}"
    url += f"&openid={openid}"
    url += "&num=10"
...
async def jiangxi(user_id: int) -> dict:
    ...
    nid = result[0]["dxx_id"]
    openid = result[0]["openid"]
    suborg = result[0]["mobile"] if result[0]["mobile"] else ''
    answer = await Answer.all().order_by("time").values()
    try:
        url = f"http://www.jxqingtuan.cn/pub/vol/volClass/index?userId={random.randint(4363000, 4364000)}"
        headers.update(
            {
                'Cookie': f'JSESSIONID={secrets.token_urlsafe(40)}',
                'Host': 'www.jxqingtuan.cn',
                'Origin': 'http://www.jxqingtuan.cn',
                'Referer': f'http://www.jxqingtuan.cn/html/h5_index.html?&accessToken={openid}',
            }
        )
        async with AsyncClient(headers=headers) as client:
...
async def zhejiang(user_id: int) -> dict:
    ...
    suborg = result[0]["mobile"] if result[0]["mobile"] else None
    answer = await Answer.all().order_by("time").values()
    try:
        headers = {
                ...
                if param_response.json()["status"] == 200:
                    ...
                    async with AsyncClient(headers=token_headers, timeout=30, max_redirects=5) as client:
                        token_response = await client.post(
                            url=token_url,
                            data=f"sign={urllib.parse.quote(content)}",
                        )
